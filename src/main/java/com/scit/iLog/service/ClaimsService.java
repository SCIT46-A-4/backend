...
public class ClaimsService {
	 ...
	 * 
	 * 클라이언트에서 전달받은 클레임 정보를 검증하고, 해당 멤버가 존재하는지 확인한 후 데이터베이스에 저장합니다.
	 * 
...
//    	ObjectUtils.isEmpty(claimsDTO)
		...
		// 2️. 멤버 조회
		MemberEntity member = memberRepository.findById(claimsDTO.getAuthorId()).orElseThrow(() -> {
		...
		ClaimEntity claimEntity = ClaimEntity.builder().author(member).title(claimsDTO.getTitle())
				.content(claimsDTO.getContent()).build();
	...
	public List<ClaimsDTO> getAllClaims() {
		List<ClaimsDTO> claimsList = claimsRepository.findAll().stream().map(ClaimsDTO::toDTO)
