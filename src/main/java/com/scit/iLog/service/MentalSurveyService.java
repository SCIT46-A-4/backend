...
public class MentalSurveyService {
        ...
        public MentalSurveyListDTO getAllMentalSurveys() {
                ...
                return new MentalSurveyListDTO(
                                mentalSurveys.stream()
                                                .map(mentalSurvey -> new MentalSurveyListItemDTO(mentalSurvey.getId(),
        ...
        public String saveMentalSurveyResponse(
                ...
                List<SectionResponseInsertDTO> sectionResponseInsertDTOS = mentalSurveyResponseInsertDTO
                                .sectionResponses();
                ...
                List<SectionResponse> sectionResponses = sectionResponseInsertDTOS.stream()
                                .map(sectionResponse -> new SectionResponse(
                                                sectionResponse.sectionTitle(),
                                                sectionResponse.questionResponses().stream()
                                                                .map(questionResponse -> new QuestionResponse(
        ...
        public MentalSurveyResponseDetailsDTO getResponseDetailsById(Long childId, String responseId) {
                MentalSurveyResponseEntity mentalSurveyResponse = mentalSurveyResponseRepository.findById(responseId)
                                .orElseThrow(() -> new EntityNotFoundException(
                                                String.format("MentalSurveyResponse 조회 실패: %s", responseId)));
                List<SectionResponseDetailsDTO> sectionResponseDetails = mentalSurveyResponse.getSectionResponses()
                                .stream()
                                .map(sectionResponse -> new SectionResponseDetailsDTO(
                                                sectionResponse.getSectionTitle(),
                                                sectionResponse.getQuestionResponses().stream().map(
                                                                questionResponse -> new QuestionResponseDetailsDTO(
        ...
        @Transactional(readOnly = true)
        public ChildMentalStatsDTO getMentalSurveyStatsBetween(Long childId, Long memberId, LocalDate startDate,
                        LocalDate endDate) {
                if (!ObjectUtils.isEmpty(startDate) && !ObjectUtils.isEmpty(endDate)) {
                        ...
                        List<ChildMentalSurveyStatPointDataDTO> surveyResponseStatPointData = mentalSurveyResponseRepository
                                        .findByChildIdAndRespondentIdAndCreatedAtBetween(childId, memberId,
                                                        startDate.atTime(23, 59, 59), endDate.atTime(23, 59, 59))
                                        .stream()
                                        .map(mentalSurveyResponse -> ChildMentalSurveyStatPointDataDTO.builder()
                                                        ...
                                                        .detailUrl(String.format(
                                                                        "/children/%d/mentalSurveys/responses/%s/details",
                                                                        childId, mentalSurveyResponse.getId()))
                                                        .build())
                                        .toList();
                        ...
                        List<ChildMentalSurveyStatPointDataDTO> surveyResponseStatPointData = mentalSurveyResponseRepository
                                        ...
                                        .map(mentalSurveyResponse -> ChildMentalSurveyStatPointDataDTO.builder()
                                                        ...
                                                        .resultScore(mentalSurveyResponse.getTotalLikertScore())
                                                        .detailUrl(String.format(
                                                                        "/children/%d/metalSurveys/responses/%s/details",
                                                                        childId, mentalSurveyResponse.getId()))
                                                        .build())
                                        .toList();
                        return new ChildMentalStatsDTO(surveyResponseStatPointData);
         ...
         * @return
         */
        private List<MentalSurveyResponseChartDTO> getSurveyData(Long childId, LocalDateTime startDate,
                        LocalDateTime endDate) {
                System.out.println("-------------------------------------------");
                Sort sort = Sort.by(asc("createdAt"));

                List<MentalSurveyResponseEntity> mentalSurveyResponseEntity
                // = mentalSurveyResponseRepository.findAllByCreatedAtBetween(startDate,
                // endDate, sort);
                                = mentalSurveyResponseRepository.findAllByChildIdAndCreatedAtBetween(childId, startDate,
                                                endDate, sort);
